...
function getOrderNumber(order) {
  if (!order) return '';
  // Use shortOrderNumber if available, otherwise last 8 chars of _id
...
function sendOrderWaitingForPickupEmail({ email, name, order }) {
  const subject = `Waiting for Pickup - ${getOrderNumber(order)}`;
  const html = `
    ...
    <div style="padding:32px;background:#f8f9fa;">
      ...
      <p style="margin-top:30px;">Order No: <b>${getOrderNumber(order)}</b></p>
    </div>
  `;
...
function sendOrderOutForDeliveryEmail({ email, name, order }) {
  const subject = `Out for Delivery - ${getOrderNumber(order)}`;
  const html = `
    ...
    <div style="padding:32px;background:#f8f9fa;">
      ...
      <p style="margin-top:30px;">Order ID: <b>${getOrderNumber(order)}</b></p>
    </div>
  `;
  return sendMail({ to: email, subject, html });
}
...
function sendOrderReturnRequestedEmail({ email, name, order }) {
  const subject = `Return Requested - ${getOrderNumber(order)}`;
  const html = `
...
export async function sendOrderConfirmedEmail({ email, name, order }) {
  const subject = `Order Confirmed - ${getOrderNumber(order)}`;
  const html = `
...
export async function sendOrderPickupRequestedEmail({ email, name, order }) {
  const subject = `Pickup Requested - ${getOrderNumber(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="https://res.cloudinary.com/dyccpu74t/image/upload/v1765529523/Asset_6_q7xkxg.png" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderPickedUpEmail({ email, name, order }) {
  const subject = `Order Picked Up - ${getOrderNumber(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="https://res.cloudinary.com/dyccpu74t/image/upload/v1765529523/Asset_6_q7xkxg.png" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderWarehouseReceivedEmail({ email, name, order }) {
  const subject = `Order Received at Warehouse - ${getOrderNumber(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="https://res.cloudinary.com/dyccpu74t/image/upload/v1765529523/Asset_6_q7xkxg.png" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderCustomStatusEmail({ email, name, order, status }) {
  const subject = `Order Update (${status}) - ${getOrderNumber(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="https://res.cloudinary.com/dyccpu74t/image/upload/v1765529523/Asset_6_q7xkxg.png" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderProcessingEmail({ email, name, order }) {
  const subject = `Order Processing - ${getOrderNumber(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="https://res.cloudinary.com/dyccpu74t/image/upload/v1765529523/Asset_6_q7xkxg.png" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderDeliveredEmail({ email, name, order }) {
  const subject = `Order Delivered - ${getOrderNumber(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="https://res.cloudinary.com/dyccpu74t/image/upload/v1765529523/Asset_6_q7xkxg.png" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderCancelledEmail({ email, name, order }) {
  const subject = `Order Cancelled - ${getOrderNumber(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="https://res.cloudinary.com/dyccpu74t/image/upload/v1765529523/Asset_6_q7xkxg.png" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderReturnedEmail({ email, name, order }) {
  const subject = `Order Returned - ${getOrderNumber(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="https://res.cloudinary.com/dyccpu74t/image/upload/v1765529523/Asset_6_q7xkxg.png" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderConfirmationEmail(orderData) {
  ...
  const subject = `Order Confirmation - ${getOrderNumber({ _id: orderId, shortOrderNumber })}`;
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <div class="order-id">
            <strong>Order No:</strong> ${getOrderNumber({ _id: orderId, shortOrderNumber })}<br>
...
export async function sendOrderShippedEmail(orderData) {
  ...
  const subject = `Order Shipped - ${getOrderNumber({ _id: orderId })}`;
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      <div class="container">
        <div class="header">
          ...
          <p>Order #${getOrderNumber({ _id: orderId })}</p>
        </div>
        <div class="content">
          <h2>Hi ${name || 'there'}! 👋</h2>
